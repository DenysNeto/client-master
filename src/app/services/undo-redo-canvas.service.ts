            ...
            switch ( last_action_obj.action ) {
                ...
                case ActionType.Unselect : {
                    ...
                    if ( (last_action_obj.parent as Group)._id === activeGroup._id ) {
                        if ( (last_action_obj.object as unknown as IGroupCustom[]).length && (last_action_obj.object as unknown as IGroupCustom[]).length > 0 ) {
                            console.log ( 'unselect 3' );
                            (last_action_obj.object as unknown as IGroupCustom[]).forEach ( ( elem ) => {
